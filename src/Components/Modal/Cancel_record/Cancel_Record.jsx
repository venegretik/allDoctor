            ...
            {showWindow ? <div className={s.Cancel_Record_full}>
                <div className={s.Cancel_Record_block}>
                    ...
                    <div className={s.Cancel_Record}>
                        <h1 className={s.Font_size40} style={{ color: config?.config.colors.color2 }}>{props.text}</h1>
                        {props.typeModal == "record" ? <p className={s.Font_size16} style={{ color: config?.config.colors.color2 }}>После удаления врача он не будет иметь доступ к вашей медицинской карте</p> : ""}
